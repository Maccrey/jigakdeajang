...
const LEGACY_SETTINGS_KEY='latebook_settings_v2';
...
function renderSummary(){const m=monthRecords(),late=m.filter(r=>r.late>0),total=late.reduce((s,r)=>s+r.fee,0),avg=late.length?Math.round(late.reduce((s,r)=>s+r.late,0)/late.length):0,max=late.reduce((a,r)=>!a||r.late>a.late?r:a,null),streak=calcStreak(m);$('#monthlyFee')&&($('#monthlyFee').textContent=money(total));$('#monthlyCount')&&($('#monthlyCount').textContent=`${late.length}회`);$('#avgLate')&&($('#avgLate').textContent=`${avg}분`);$('#maxLate')&&($('#maxLate').textContent=`${max?.late||0}분`);$('#maxLateDate')&&($('#maxLateDate').textContent=max?fmtFullDate(new Date(max.datetime)):'기록 없음');$('#streakLate')&&($('#streakLate').textContent=`${streak}일`);$('#donutTotal')&&($('#donutTotal').textContent=money(total));$('#recordSummary')&&($('#recordSummary').textContent=`${m.length}건`);renderBars(m);renderDonut(late,total);renderTable(m)}
...
function renderTable(m){const body=$('#recordBody');if(!body)return;body.innerHTML='';const rows=[...m].sort((a,b)=>new Date(b.datetime)-new Date(a.datetime));if(!rows.length){body.innerHTML='<tr class="empty-row"><td colspan="7">이번 달 기록이 없습니다.</td></tr>';return}rows.forEach(r=>{const d=new Date(r.datetime),tr=document.createElement('tr');if(r.late>0)tr.className='late-row';tr.innerHTML=`<td>${fmtDate(d)}</td><td>${esc(r.name)}</td><td>${fmtTime(d)}</td><td>${r.startTime||settings.startTime}</td><td>${r.late}분</td><td>${money(r.fee)}</td><td><button class="delete-btn" data-id="${r.id}">✕</button></td>`;body.appendChild(tr)});$$('.delete-btn').forEach(b=>b.onclick=()=>{records=records.filter(r=>r.id!==b.dataset.id);localStorage.setItem(STORAGE_KEY,JSON.stringify(records));renderSummary()})}
function buildBackupPayload(){return{app:'지각대장',formatVersion:2,exportedAt:new Date().toISOString(),settings:{...defaultSettings,...settings,employees:[...(settings.employees||[])]},records:[...records],calendarEvents:[...calendarEvents]}}
...
function refreshBackupUi(){$('#backupCount')&&($('#backupCount').textContent=`기록 ${records.length}건`)}
...
function bind(){
  ...
  if(form)form.addEventListener('submit',e=>{e.preventDefault();const name=$('#nameInput').value,dt=new Date($('#dateTimeInput').value);if(!name){alert('직원을 선택해 주세요.');return}if(Number.isNaN(dt.getTime()))return;const duplicate=records.some(r=>r.name===name&&localDateKey(new Date(r.datetime))===localDateKey(dt));if(duplicate){alert(`${name}님은 ${localDateKey(dt)}에 이미 출근기록이 있습니다.\n동일한 날짜에는 중복 기록할 수 없습니다.`);return}const overrideStart=($('#appliedStartTime')?.value||settings.startTime||'09:00');const{late,fee,usedStartTime}=calcLate(dt,overrideStart),r={id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),name,datetime:dt.toISOString(),startTime:usedStartTime,feePerMinute:settings.feePerMinute,late,fee};records.push(r);localStorage.setItem(STORAGE_KEY,JSON.stringify(records));updateResult(r);renderSummary()});
